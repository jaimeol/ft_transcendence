...
export async function mount(el: HTMLElement, ctx: Ctx) {
  ...
  async function viewTournament(tournamentId: number) {
    ...
    if (!r.ok) { alert(r.data?.error || 'Error loading tournament'); return; }
  ...
  function renderBracketPreviewHtml(t: BackendTournament) {
    if (!t.matches || t.matches.length === 0) return '<div class="text-center text-white/60 py-4">No matches yet</div>';
    const rounds: Record<number, any[]> = {};
  ...
  async function checkAndShowAdvanceButton(tournamentId: number) {
    ...
    if (!r.ok) { alert(r.data?.error || 'Error checking'); return; }
    const data = r.data;
    if (data.canAdvance) {
  ...
  // typed selectors
  function $HTMLSelectElement(sel: string) { return el.querySelector(sel) as HTMLSelectElement | null; }
  function $HTMLElement(sel: string) { return el.querySelector(sel) as HTMLElement | null; }
}
