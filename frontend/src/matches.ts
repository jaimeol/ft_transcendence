...
export async function mount(el: HTMLElement, ctx: Ctx) {
	...
	el.innerHTML = `
			...
			<div class="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
				...
				<div class="flex items-center gap-3">
					<div class="bg-white/5 border border-white/10 px-2 py-1 rounded-full text-xs backdrop-blur">
		  			...
		  			<a href="/profile" class="hidden sm:inline-flex items-center bg-white/10 hover:bg-white/15 border border-white/10 px-3 py-1.5 rounded-lg text-xs">
		...
		<main class="max-w-5xl mx-auto px-4 py-8">
			<h1 class="text-2xl font-bold mb-4">${t("home.recentMatches") ?? "Partidos"}</h1>
			...
			<!-- Controles -->
	  		<div class="flex flex-wrap items-center gap-2 mb-4">
				...
				<select id="filter-result" class="bg-black border border-white/10 rounded-lg px-3 py-2 text-white/80 ">
					...
					<option value="L">${t("lose") ?? "Derrota"}</option>
				</select>
			</div>
	...
	try {
		...
		listEl.innerHTML = `<div class="text-rose-400">${t("error_loading") ?? "No se pudieron cargar los partidos."}</div>`;
		return;
	}
	...
	function badge(res: "W" | "L" | "D") {
		const cls = res === "W" ? "bg-emerald-600/70" : res === "L" ? "bg-rose-600/70" : "bg-zinc-600/70";
		const txt = res === "W" ? (t("win") ?? "Victoria")
			: res === "L" ? (t("lose") ?? "Derrota")
			: (t("draw") ?? "Empate");
		return `<span class="ml-2 inline-flex text-xs px-2 py-0.5 rounded ${cls}">${txt}</span>`;
	}
	...
	function rowHTML(m: Match, res: "W" | "L" | "D", oppName: string) {
		...
		if (g === "pong") {
			...
			const scoreHTML  = sc
				...
				: `<div class="text-sm opacity-70">${t("no_score") ?? "Sin marcador"}</div>`;
	...
	async function render(reset = false) {
		...
		if (filtered.length === 0) {
			listEl.innerHTML = `<div class="text-white/60">${t("no_matches") ?? "Aún no hay partidas"}</div>`;
