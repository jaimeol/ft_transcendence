...
// Declarar Chart como variable global
...
type Me = { id: number | null; display_name?: string; email?: string };
...
`.trim();
  ...
  // Utils
  const $ = <T extends HTMLElement = HTMLElement>(s: string, r: Document | HTMLElement = document) =>
  ...
  // Filters
  const filterGameSel = $("#filter-game") as HTMLSelectElement;
  const filterRangeSel = $("#filter-range") as HTMLSelectElement;

  // Charts instances
  let chartWDL: any = null;
  let chartGoals: any = null;
  let chartTimeline: any = null;

  function safeDetails(m: Match): any | null {
    try { return m.details ? JSON.parse(m.details) : null; } catch { return null; }
  }
  
  function gameKey(m: Match): "pong" | "tictactoe" | "unknown" {
    ...
    if (g === "pong" || g.includes("pong")) return "pong";
    if (g.includes("tictactoe") || g === "ttt") return "tictactoe";
    return "unknown";
  }
  
  function inRange(m: Match, range: string): boolean {
  ...
  function resultOf(m: Match): "win" | "draw" | "loss" {
    const d = safeDetails(m);
    if (m.is_draw || d?.is_draw) return "draw";
    if (m.winner_id === meUser!.id) return "win";
    return "loss";
  }

  // Goals extraction for pong-like
  function extractGoals(m: Match): { for: number; against: number } | null {
    const d = safeDetails(m);
    if (!d) return null;
    ...
    if (s && typeof s === "object") {
      ...
      if (typeof s.left === "number" && typeof s.right === "number") {
        ...
        if (m.winner_id === meUser!.id) {
          if (s.left > s.right) return { for: s.left, against: s.right };
          if (s.right > s.left) return { for: s.right, against: s.left };
        } else {
          if (s.left > s.right) return { for: s.right, against: s.left };
          if (s.right > s.left) return { for: s.left, against: s.right };
        }
      }
    }
    ...
    if (typeof d.score_left === "number" && typeof d.score_right === "number") {
      ...
      if (myLeft === true) return { for: d.score_left, against: d.score_right };
      if (myLeft === false) return { for: d.score_right, against: d.score_left };
      
      // Fallback con ganador
      if (m.winner_id === meUser!.id) {
        if (d.score_left > d.score_right) return { for: d.score_left, against: d.score_right };
        if (d.score_right > d.score_left) return { for: d.score_right, against: d.score_left };
      } else {
        if (d.score_left > d.score_right) return { for: d.score_right, against: d.score_left };
        if (d.score_right > d.score_left) return { for: d.score_left, against: d.score_right };
      }
    }
  ...
  function computeStats(gameFilter: string, rangeFilter: string) {
    const rows = matches.filter(m => {
      const g = gameKey(m);
      const okGame = gameFilter === "all" ? g !== "unknown" : g === (gameFilter as any);
      return okGame && inRange(m, rangeFilter);
    });

    let wins = 0, draws = 0, losses = 0;
    let gf = 0, ga = 0;
    const timeSeries: { label: string; val: number }[] = [];
    ...
    for (const m of sorted) {
      ...
      if (g === "pong") {
        ...
        if (scor) { gf += scor.for; ga += scor.against; }
      }
    }
  ...
  function updateKPIs(s: ReturnType<typeof computeStats>, gameFilter: string) {
  ...
  function drawCharts(s: ReturnType<typeof computeStats>, gameFilter: string) {
    // Destroy existing
    chartWDL?.destroy(); chartGoals?.destroy(); chartTimeline?.destroy();

    // WDL doughnut
    chartWDL = new Chart($("#chart-wdl") as HTMLCanvasElement, {
      type: "doughnut",
      data: {
    ...
    const goalsCanvas = $("#chart-goals") as HTMLCanvasElement;
    ...
    // Timeline line
    chartTimeline = new Chart($("#chart-timeline") as HTMLCanvasElement, {
      type: "line",
      data: {
  ...
  function renderTable(rows: Match[], gameFilter: string) {
    const tb = $("#tbl-body")!;
    const max = 15;
    const lines = rows
      ...
      .map(m => {
        ...
        const badge =
          r === "win" ? `<span class="text-green-400">${ctx.t("win")}</span>` :
