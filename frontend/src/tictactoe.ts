...
export async function mount(el: HTMLElement, { t, api, navigate }: Ctx) {
  await initializeLanguages();
	...
	<div id="pvp-login-overlay" class="fixed inset-0 hidden flex items-center justify-center bg-black/60 z-50">
		...
		<h2 class="text-xl font-semibold mb-4" data-translate="pvp.second_player">${
...
+			${t("pvp.start_match") ?? "Empezar partida"}
	...
	const googleSecondHost = el.querySelector<HTMLElement>("#google-second-host");
  ...
  if (googleSecondHost) {
    renderGoogleSecondButton(
      ...
      (player) => {
        ...
        pvpReady = true;
        (document.querySelector("#pvp-login-overlay") as HTMLElement)?.classList.add("hidden");
        (document.querySelector("#board-blocker") as HTMLElement)?.classList.add("hidden");
