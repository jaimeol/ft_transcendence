...
export function mountChat(host: HTMLElement, ctx: Ctx){
		...
		if (m.kind === 'invite') {
			...
			if (isMine) {
				...
				const button = h('button', {
					class: `px-3 py-1 rounded text-black font-semibold text-sm whitespace-nowrap ${
						status === 'pending'
				...
				if (status === 'pending') {
					...
					return h('button', {
						...
						onclick: async () => {
							...
							if (!accepted) {
								try {
									const response = await api<{ message: Msg }>(`/api/chat/accept-invite/${m.id}`, {
										method: 'POST'
									});
					...
					return h('div', { class: `${baseClass} bg-emerald-700/60 opacity-70` },
						(ctx.t("chat.invite_accepted") ?? "¡Invitación aceptada! Ve al ordenador del anfitrión.")
					);
				}
			}
		}
	...
	function updateMessageInUI(m: Msg) {
		if (!m.id) return;
		const wrapper = $(`[data-msg-id="${m.id}"]`, host);
		if (!wrapper) return; // El mensaje no está cargado/visible
		...
		if (timeEl) wrapper.append(timeEl); // Re-añadir el timestamp
	}
	...
	async function updateChatTexts() {
		...
		if (peersHeader) peersHeader.textContent = ctx.t("chat.friends") ?? "Amigos";
		
		const sendButton = host.querySelector('#chat-input + button'); // Botón "Enviar"
		if (sendButton) sendButton.textContent = ctx.t("chat.send") ?? "Enviar";
	...
	async function sendInvite(to: number) {
