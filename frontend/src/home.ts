...
export async function mount(el: HTMLElement, ctx: Ctx) {
	...
	const $ = <T extends HTMLElement = HTMLElement>(sel: string) => el.querySelector(sel) as T | null;
	...
	async function loadFriendsCount() {
		try {
			...
			if (badge) badge.textContent = `${count} ${word}`;
		} catch {}
	}
	...
	function perspectiveScore(m: Match, myId: number): { you: number; rival: number } | null {
		const d = safeDetails(m);
		if (!d?.score && !Number.isFinite(d?.score_user)) return null;
	...
	async function getUserName(userId: number): Promise<string> {
		if (!userId || userId <= 0) return ctx.t("AI");
		try {
