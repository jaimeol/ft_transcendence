...
async function fetchClientId(): Promise<string> {
  ...
  if (!r.ok) throw new Error("No se pudo obtener /api/config");
...
export async function initGoogleUI(root: HTMLElement, navigate: (p: string, o?: {replace?: boolean}) => void) {
  // Refs **dentro** de root (existen tras mount)
  const btn = root.querySelector("#google-btn") as HTMLButtonElement | null;
  const label = root.querySelector("#google-label") as HTMLSpanElement | null;
  const gIcon = root.querySelector("#google-g-icon") as SVGElement | null;
  const spinner = root.querySelector("#google-spinner") as SVGElement | null;
