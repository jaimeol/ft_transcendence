...
export async function mount(el, ctx) {
    ...
    if (isAI && !players) {
        ...
        if (playersOverlay) {
            ...
            playersOverlay.querySelectorAll('.players-btn').forEach(btn => {
    ...
    if (isAI) {
        ...
        const pressKeyWithDuration = (key) => {
        ...
        const updateAI = () => {
            ...
            let reactionDelay = 0;
            if (difficulty === 'easy')
                reactionDelay = Math.random() < 0.3 ? 1 : 0; // 30% de fallar una reacción
            else if (difficulty === 'medium')
                reactionDelay = Math.random() < 0.15 ? 1 : 0; // 15% de fallar una reacción
            else if (difficulty === 'hard')
                reactionDelay = Math.random() < 0.05 ? 1 : 0; // 5% de fallar una reacción
            ...
            if (keyToPress !== aiPressedKey) {
                if (aiPressedKey)
                    simulateKeyRelease(aiPressedKey);
                if (keyToPress)
                    pressKeyWithDuration(keyToPress);
        ...
        const updateAIHorizontal = () => {
            ...
            let reactionDelay = 0;
            if (difficulty === 'easy')
                reactionDelay = Math.random() < 0.3 ? 1 : 0;
            else if (difficulty === 'medium')
                reactionDelay = Math.random() < 0.15 ? 1 : 0;
            else if (difficulty === 'hard')
                reactionDelay = Math.random() < 0.05 ? 1 : 0;
            if (reactionDelay)
                return;
            let keyToPress = null;
            ...
            if (difficulty === 'medium' && Math.random() < 0.1) {
                // 10% de error
                if (diff < -DEAD_ZONE && topPaddleX + hPaddleWidth < canvas.width)
                    keyToPress = 'd';
                else if (diff > DEAD_ZONE && topPaddleX > 0)
                    keyToPress = 'a';
            }
            else {
                // Movimiento normal
                if (diff < -DEAD_ZONE && topPaddleX > 0)
                    keyToPress = 'a';
                else if (diff > DEAD_ZONE && topPaddleX + hPaddleWidth < canvas.width)
                    keyToPress = 'd';
            }
