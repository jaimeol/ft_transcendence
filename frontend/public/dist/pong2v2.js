...
export async function mount(el, ctx) {
    ...
    async function saveMatchIfNeeded() {
        try {
            ...
            const exportAI = async (opts) => {
                ...
                if (opts.as_user_id)
                    body.as_user_id = opts.as_user_id;
                await post(body);
            };
    ...
    async function handlePlayerLogin(email, password, successCallback, errorElement) {
        try {
    ...
    {
        const host2 = el.querySelector('#pvp-google-host2');
        if (host2) {
            void renderGoogleSecondButton(host2, (player) => {
                secondPlayer = player;
                pvpReady = true;
                pvpOverlay.classList.add('hidden');
                if (mode === 'pvp') {
    ...
    if (mode === 'pvp') {
        const form3 = el.querySelector('#pvp-login-form3');
        const errorEl3 = $('#pvp-login-error3');
        form3.addEventListener('submit', async (ev) => {
            ev.preventDefault();
            const email = (el.querySelector('#pvp-email3')).value;
            const password = (el.querySelector('#pvp-password3')).value;
            await handlePlayerLogin(email, password, (player) => {
                thirdPlayer = player;
                pvpReady3 = true;
        ...
        {
            const host3 = el.querySelector('#pvp-google-host3');
            if (host3) {
                void renderGoogleSecondButton(host3, (player) => {
                    thirdPlayer = player;
                    pvpReady3 = true;
                    $('#pvp-login-overlay3').classList.add('hidden');
                    $('#pvp-login-overlay4').classList.remove('hidden');
        ...
        const form4 = el.querySelector('#pvp-login-form4');
        const errorEl4 = $('#pvp-login-error4');
        form4.addEventListener('submit', async (ev) => {
            ev.preventDefault();
            const email = (el.querySelector('#pvp-email4')).value;
            const password = (el.querySelector('#pvp-password4')).value;
            await handlePlayerLogin(email, password, (player) => {
                fourthPlayer = player;
                pvpReady4 = true;
        ...
        {
            const host4 = el.querySelector('#pvp-google-host4');
            if (host4) {
                void renderGoogleSecondButton(host4, (player) => {
                    fourthPlayer = player;
                    pvpReady4 = true;
                    $('#pvp-login-overlay4').classList.add('hidden');
                    blocker?.classList.add('hidden');
    ...
    const onKeyDown = (e) => {
        ...
        if (mode === 'ai' && e.isTrusted && (e.key === keyRightUp || e.key === keyRightDown ||
    ...
    const setAIHDirection = (dir) => {
        if (aiHDir === dir)
            return;
        if (aiHDir === 'left')
            press(keyTopLeft, false);
        if (aiHDir === 'right')
            press(keyTopRight, false);
        aiHDir = dir;
        if (aiHDir === 'left')
            press(keyTopLeft, true);
        if (aiHDir === 'right')
            press(keyTopRight, true);
    };
    // Predicción vertical (bola hacia pala derecha)
    const predictBallYAtRight = () => {
        if (ballSpeedX <= 0) {
        ...
        let err = 0;
        if (difficulty === 'easy')
            err = canvas.height * 0.14;
        else if (difficulty === 'medium')
            err = canvas.height * 0.06;
        else
            err = canvas.height * 0.02;
        if (Math.abs(vy) < 1)
            err *= 0.5;
    ...
    const predictBall2XAtTop = () => {
        ...
        while (simY > targetY && it++ < maxIt) {
            ...
            else if (simX >= canvas.width - ballRadius) {
                simX = canvas.width - ballRadius;
                vx = -Math.abs(vx);
            }
        }
        ...
        let err = 0;
        if (difficulty === 'easy')
            err = canvas.width * 0.14;
        else if (difficulty === 'medium')
            err = canvas.width * 0.06;
        else
            err = canvas.width * 0.02;
        if (Math.abs(vx) < 1)
            err *= 0.5;
        predicted += (Math.random() * 2 - 1) * err;
        const margin = 8;
    ...
    // Guiado continuo
    const steerVertical = () => {
        if (!gameRunning || gameOver1 || aiVTargetY == null) {
            setAIVDirection('none');
            return;
        }
        ...
        if (Math.abs(diff) <= localDead)
            ...
            setAIVDirection(diff < 0 ? 'up' : 'down');
    };
    const steerHorizontal = () => {
        if (!gameRunning || gameOver2 || aiHTargetX == null) {
            setAIHDirection('none');
            return;
        }
        ...
        if (ball2SpeedY < 0) {
            if (distY < 160)
                localDead = Math.min(localDead, 2);
            if (distY < 90)
                localDead = 0;
        }
        if (Math.abs(diff) <= localDead)
            ...
            setAIHDirection(diff < 0 ? 'left' : 'right');
    };
    const updateAIV = () => {
        ...
        if (ballSpeedX > 0) {
            ...
            if (aiVTargetY == null)
                aiVTargetY = rightPaddleY + paddleHeight / 2;
        }
        steerVertical();
    };
    const updateAIH = () => {
        ...
        if (ball2SpeedY < 0) {
            ...
            if (aiHTargetX == null)
                aiHTargetX = topPaddleX + hPaddleWidth / 2;
        }
        steerHorizontal();
    };
    /* -------------------------
    ...
    function update() {
        ...
        if (keys[keyLeftDown] && leftPaddleY + paddleHeight < canvas.height)
            leftPaddleY += paddleSpeed;
        // Actualizar paddle derecho (controlado por IA o jugador 3)
        if (keys[keyRightUp] && rightPaddleY > 0)
            rightPaddleY -= paddleSpeed;
        if (keys[keyRightDown] && rightPaddleY + paddleHeight < canvas.height)
            rightPaddleY += paddleSpeed;
        ballX += ballSpeedX;
        ballY += ballSpeedY;
    ...
    (function loop() {
        ...
        if (mode === 'ai') {
            ...
            if (gameRunning && aiVTargetY != null)
                steerVertical();
            ...
            if (gameRunning && aiHTargetX != null)
                steerHorizontal();
        }
        if (gameRunning) {
            update();
