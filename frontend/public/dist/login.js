...
export async function mount(el, ctx) {
    ...
    <div class="min-h-screen bg-gradient-to-br from-black via-zinc-900 to-black text-white relative overflow-hidden">
        ...
        <main class="min-h-screen grid place-items-center px-4">
            <section class="w-full max-w-md p-8 rounded-2xl bg-zinc-900/60 backdrop-blur">
                <h1 id="login-title" class="text-3xl font-bold mb-8 text-center">${t("login") ?? "Iniciar sesión"}</h1>
                ...
                <form id="login-form" class="space-y-6" autocomplete="on">
                    <div class="space-y-2">
                        <label for="email" id="email-label" class="block text-sm text-white">${t("field-email") ?? "Email"}</label>
                    ...
                    <div class="space-y-2">
                        <label for="password" id="password-label" class="block text-sm text-zinc-300">${t("field-password") ?? "Contraseña"}</label>
                    ...
                    <div class="mt-2 space-y-2">
                        <button id="google-btn" type="button" aria-label="Continuar con Google" aria-busy="false"
                            ...
                            <span id="google-label">${t("google-login") ?? "Continuar con Google"}</span>
                    ...
                    <p class="text-sm text-center text-zinc-400 mt-2">
                        <span id="no-account-label">${t("no_account") ?? "¿No tienes cuenta?"}</span>
                        <a class="ml-1 text-indigo-400 hover:text-indigo-300 underline" href="/register" id="register-link">${t("register") ?? "Regístrate"}</a>
                    </p>
                </form>
            </section>
        </main>
    ...
    window.addEventListener("languageChanged", () => {
        qs("#login-title").textContent = t("login") ?? "Iniciar sesión";
        qs("#email-label").textContent = t("field-email") ?? "Email";
        qs("#password-label").textContent = t("field-password") ?? "Contraseña";
        qs("#submitBtn").textContent = t("login") ?? "Entrar";
        qs("#google-label").textContent = t("google-login") ?? "Continuar con Google";
        qs("#no-account-label").textContent = t("no_account") ?? "¿No tienes cuenta?";
        qs("#register-link").textContent = t("register") ?? "Regístrate";
        // Actualizar placeholder del email
        const emailInput = qs("#email");
        if (emailInput) {
            emailInput.placeholder = t("email-placeholder") ?? "tu@email.com";
        }
    });
}
