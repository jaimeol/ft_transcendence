...
export async function mount(el, { t, api, navigate }) {
	...
	<div id="pvp-login-overlay" class="fixed inset-0 hidden flex items-center justify-center bg-black/60 z-50">
		...
		<h2 class="text-xl font-semibold mb-4" data-translate="ttt.second_player">${t("ttt.second_player") ?? "Segundo jugador"}</h2>
	...
	`;
    const canvas = el.querySelector("#ticTacToe");
    const svg = el.querySelector("#ttt-anim");
    const statusEl = el.querySelector("#status");
    const resetBtn = el.querySelector("#resetBtn");
    const playAgainBtn = el.querySelector("#playAgainBtn");
    const closeOverlayBtn = el.querySelector("#closeOverlayBtn");
    const winnerOverlay = el.querySelector("#winnerOverlay");
    const winnerText = el.querySelector("#winnerText");
    const blocker = el.querySelector("#board-blocker");
