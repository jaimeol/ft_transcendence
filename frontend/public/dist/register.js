...
export async function mount(el, ctx) {
  ...
  `;
    ...
    const setError = (m) => { const box = qs("#err"); if (box)
        box.textContent = m; };
    const setLoading = (v) => {
    ...
    (function setupEmailReadOnly() {
        ...
        setTimeout(() => { if (document.activeElement === email)
            enable(); }, 0);
    })();
    ...
    qs("#form-register")?.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (msg === "409")
                setError(`${t("email.name.in-use")}`);
            else if (msg === "400")
                setError(`${t("register.invalid.password")}`);
            else
                setError(`${t("register.unexpected")}`);
            setLoading(false);
        }
    });
    import("./google.js")
    ...
    window.addEventListener("languageChanged", () => {
        ...
        const displayNameInput = qs('input[name="display_name"]');
        if (displayNameInput)
            displayNameInput.placeholder = t("username-placeholder") || "Tu nombre público";
        const firstNameInput = qs('input[name="first_name"]');
        if (firstNameInput)
            firstNameInput.placeholder = t("name-placeholder") || "Nombre";
        const lastNameInput = qs('input[name="last_name"]');
        if (lastNameInput)
            lastNameInput.placeholder = t("last_name-placeholder") || "Apellido";
        const passwordInput = qs('input[name="password"]');
        if (passwordInput)
            passwordInput.placeholder = t("pass-placeholder") || "Contraseña (mín. 8)";
