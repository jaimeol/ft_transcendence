...
function mountUI() {
    ...
    panel.append(
    // barra cabecera
    h('div', { class: 'max-w-6xl mx-auto px-3 py-2 flex items-center gap-3' }, h('strong', {}, 'Chat'), h('span', { id: 'chat-peer-name', class: 'opacity-80 text-sm' }, '—'), h('div', { id: 'chat-actions', class: 'ml-auto flex gap-2' })), 
    // cuerpo: lista de peers + conversación
    h('div', { class: 'max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-[260px_1fr] gap-3 px-3 pb-3' }, h('aside', { class: 'bg-white/5 rounded-xl p-2 max-h-64 overflow-auto' }, h('div', { id: 'chat-peers', class: 'space-y-1' }, 'Cargando...')), h('div', { class: 'bg-white/5 rounded-xl p-2 flex flex-col' }, h('div', { id: 'chat-messages', class: 'flex-1 overflow-auto flex flex-col' }), h('div', { class: 'mt-2 flex gap-2' }, h('input', { id: 'chat-input', class: 'flex-1 bg-zinc-800 rounded p-2', placeholder: 'Escribe un mensaje…', disabled: 'true',
